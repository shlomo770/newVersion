...
export default function MapDimmerAuto({
  ...
  const ensure = () => {
    ...
    const baseSourceIds = Object.entries(sources)
      .filter(([, s]) => (s as { type?: string }).type !== 'geojson')
      .map(([k]) => k);
    let lastBaseIdx = -1;
    for (let i = 0; i < layers.length; i++) {
      const lyr = layers[i];
      if (lyr.id === id) continue;
      if (lyr.type === 'background' || (lyr.source && baseSourceIds.includes(lyr.source))) {
        lastBaseIdx = i;
      }
    }
    const currentIdx = layers.findIndex((l) => l.id === id);
    let shouldBeIdx = currentIdx;
    if (lastBaseIdx >= 0 && lastBaseIdx < layers.length - 1) shouldBeIdx = lastBaseIdx + 1;
    else shouldBeIdx = layers.length - 1;
