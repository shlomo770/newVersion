...
/* ------------------------------------------------------------------ */
/*  Per-JSON-path id builders (imported path overlays)                  */
/* ------------------------------------------------------------------ */

export const JSON_PATH_PREFIXES = {
  source: 'json-path-',
  lineLayer: 'json-path-line-',
  pointsSource: 'json-path-points-',
  pointsLayer: 'json-path-point-',
  labelLayer: 'json-path-label-',
} as const;

export function jsonPathSourceId(pathId: string): string {
  return `${JSON_PATH_PREFIXES.source}${pathId}`;
}
export function jsonPathLineLayerId(pathId: string): string {
  return `${JSON_PATH_PREFIXES.lineLayer}${pathId}`;
}
export function jsonPathPointsSourceId(pathId: string): string {
  return `${JSON_PATH_PREFIXES.pointsSource}${pathId}`;
}
export function jsonPathPointsLayerId(pathId: string): string {
  return `${JSON_PATH_PREFIXES.pointsLayer}${pathId}`;
}
export function jsonPathLabelLayerId(pathId: string): string {
  return `${JSON_PATH_PREFIXES.labelLayer}${pathId}`;
}
