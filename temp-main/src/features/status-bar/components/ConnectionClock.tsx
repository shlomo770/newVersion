...
function selectedModeLabelHe(mode: SelectedModeE | null): string {
  if (mode == null) return '—';
  if (mode === SelectedModeE.Mission) return 'מבצעי';
  if (mode === SelectedModeE.Training) return 'אימון';
  if (mode === SelectedModeE.Planning) return 'תכנון';
  if (mode === SelectedModeE.Maintenance) return 'תחזוקה';
  return String(mode);
}
...
function ConnectionClock() {
  ...
  return (
    <div className={styles.sectionEnd}>
      ...
      <div className={styles.modeLabel}>מוד {selectedModeLabelHe(selectedMode)}</div>
      <div
