...
export class TouchGestureRecognizer {
  ...
  onTap = (_e: TouchEvent, _mapCoordinates?: Coordinates) => {
    HapticFeedback.success();
  };

  onDoubleTap = (_e: TouchEvent, _mapCoordinates?: Coordinates) => {
    HapticFeedback.trigger('medium');
  };

  onLongPress = (_e: TouchEvent, _mapCoordinates?: Coordinates) => {
    HapticFeedback.trigger('heavy');
  };

  onSwipe = (_e: TouchEvent, _delta: { x: number; y: number }, _mapCoordinates?: Coordinates) => {
    // Override in specific components
  };

  onPinch = (_e: TouchEvent, _scale: number, _mapCoordinates?: Coordinates) => {
    // Override in specific components
  };
