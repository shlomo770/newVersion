...
const isLatLng = (v: unknown): v is { lat: number | string; lng: number | string } =>
  typeof v === 'object' &&
  v !== null &&
  'lat' in v &&
  'lng' in v &&
  isNumberOrString((v as { lat: unknown }).lat) &&
  isNumberOrString((v as { lng: unknown }).lng);

type Validator<T> = (payload: T) => boolean;

const validators: {
  [K in OutboundMessageName]?: Validator<OutboundMessageMap[K]>;
} = {
  ...
  [WsMessageName.LoadMission]: (p: OutboundMessageMap[WsMessageName.LoadMission]) =>
    isString(p.mission_id) && p.mission_id.length > 0,
  ...
  [WsMessageName.SaveMission]: (p: OutboundMessageMap[WsMessageName.SaveMission]) =>
    isString(p.mission_id) && isString(p.mission_name) && isString(p.entities),
